      ...
      <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
        ...
        <a href="#hero" className="flex items-center gap-3 group">
          <div className="w-10 h-10 rounded-full border border-[#c8a96e]/60 flex items-center justify-center group-hover:border-[#c8a96e] transition-colors">
            <span className="text-[#c8a96e] font-display text-lg font-semibold leading-none">P</span>
          </div>
          <div>
        ...
        <ul className="hidden md:flex items-center gap-8">
          {navLinks.map((link) => (
            <li key={link.label}>
              <a
                href={link.href}
                className="text-white/70 hover:text-[#c8a96e] text-sm tracking-widest uppercase font-light transition-colors duration-300 relative group"
              >
                {link.label}
        ...
        >
          <span className={`block w-6 h-px bg-[#c8a96e] transition-all duration-300 ${menuOpen ? 'rotate-45 translate-y-2' : ''}`} />
          <span className={`block w-6 h-px bg-[#c8a96e] transition-all duration-300 ${menuOpen ? 'opacity-0' : ''}`} />
          <span className={`block w-6 h-px bg-[#c8a96e] transition-all duration-300 ${menuOpen ? '-rotate-45 -translate-y-2.5' : ''}`} />
        </button>
      </div>
